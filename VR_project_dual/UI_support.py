import cv2 as cv
import copy
from perspective_spport import *


# 2幅图的拼接
def part_joint(left, right, set_yaw, pitch=10, yaw=0):
    yaw_gap_left = set_yaw/2
    yaw_gap_right = set_yaw/2
    ...
    # 开始变换
    trans_left = get_perspective(camera_map, view_map[0])
    trans_right = get_perspective(camera_map, view_map[1])
    ...
    # cv.waitKey(0)
    # 手动拼接部分
    offset_1 = caclulate_offset(camera_map, [trans_right, trans_left], size, set_yaw)
    width = a_left.shape[1] + a_right.shape[1] - offset_1[0]
    joint = np.zeros([int(a_left.shape[0]+25), width, 3], np.uint8)  # 创建新图
    # offset 坐标计算
    left_position = [0, 20]  # 左图的左上角坐标位置，cv格式，我们最好把合成图上面和下面都留一些黑边
    right_position = [a_left.shape[1], 20]  # 默认右图左上角坐标位置，cv格式
    right_position[0] = right_position[0] - offset_1[0]  # 对拼接图进行位移调整offset，如没有调整就是整幅拼接
    right_position[1] = right_position[1] - offset_1[1]
    right_position = pos_limiting(right_position, a_left.shape[0] + 25)
    # 进行joint
...
def pic_auto_cali(left_input, mid_input, winname):
    ...
    for i in range(30):
        ...
        cv.waitKey(20)
    return x_mean_differ_left, y_mean_differ_left, merge
